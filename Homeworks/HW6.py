...
print('Newton: took this many seconds:',elapsed)
...
print('Lazy Newton: took this many seconds:',elapsed)
...
def evalF(x):
    ...
    F[2] = -(x[0] ** 2) - 0.1 * x[1] ** 2 + x[2] - 1
    return F

def evalJ(x):
    J = np.array([[1 - x[2]*x[1]*np.sin(x[0]*x[1]*x[2]), -x[0]*x[2]*np.sin(x[0]*x[1]*x[2]), -x[0]*x[1]*np.sin(x[0]*x[1]*x[2])],
...
tol = 5*1e-2
t = time.time()
[xstar, gval, ier, its1] = SteepestDescent(x0, tol, Nmax)
[xstar,ier,its2] = Newton(xstar,1e-6,Nmax)
elapsed = time.time()-t
